...
category: itsi
description: >-
  Diagnose and fix ITSI search-head saturation — skipped searches, KPIs going N/A under load, a
  sluggish Service Analyzer. Stock ITSI ships with its scheduler close to maxed out: a dozen
  housekeeping searches run every minute by default, entity-import modular inputs run whether or
  not you use them, and shipped content packs add dozens of base searches that run every five
  minutes with or without consumers. Covers the diagnostic probes (skips by search, app and hour,
  cumulative-runtime hogs, orphaned base searches, services with no entities bound, stray
  acceleration jobs), the catalogue of defaults worth rescheduling and what each actually does,
  the disable-versus-stretch-the-cron decision, the namespace gotcha that these live in the itsi
  app rather than SA-ITOA, how to verify the change moved the needle, and what not to disable
  however unused it looks. Use when the scheduler is saturated, searches are skipped, KPIs go N/A
  under load, or somebody concludes the instance is undersized.
